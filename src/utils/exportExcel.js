...
export const exportToExcel = (state) => {
  ...
  oBanner('📊  Wealth For FIRE — Financial Overview', 'Exported on ' + dateStr);
  ...
  oBlank();
  const footerIdx = oR([emptyCell(), emptyCell(), cell(`Generated by Wealth For FIRE · ${new Date().toLocaleString('en-IN')}`, { font: font({ sz: 9, italic: true, color: C.slate600 }), alignment: align('right', 'center') })]);
  ...
  if (msgs.length === 0) {
    ...
    msgs.forEach((msg) => {
      ...
      chatRows.push([
        ...
        cell(isUser ? '👤 You' : '🤖 Wealth For FIRE AI', {
          fill: fill(msgBg),
    ...
    chatRows.push([cell(`Total: ${msgs.length} message(s) from ${allMessages.filter(m => !m.isWelcome).length} in history · Wealth For FIRE Export`, {
      font: font({ sz: 9, italic: true, color: C.slate600 }),
      alignment: align('right', 'center'),
    })]);
  ...
  XLSXStyle.writeFile(wb, `Wealth_For_FIRE_Report_${exportDate}.xlsx`);
};
