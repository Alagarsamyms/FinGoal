import { PieChart, Activity, Wallet, Target, Flame, Settings, ShieldCheck, Sparkles, X, Sun, Moon } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="mt-auto pt-6 border-t border-slate-200 dark:border-slate-800">
        <button 
          onClick={toggleTheme}
          className="w-full flex items-center justify-between px-4 py-2 mb-4 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
        >
          <span>{theme === 'light' ? 'Dark Mode' : 'Light Mode'}</span>
          {theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}
        </button>

        <div id="sync-status-react" className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-500 mb-4 font-medium px-2">
          <div className="w-2 h-2 rounded-full bg-amber-500"></div>
          Local Storage
        </div>
        <button id="auth-btn-react" className="w-full flex items-center justify-center gap-2 px-4 py-2 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
