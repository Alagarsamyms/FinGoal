    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm min-h-[400px] flex flex-col">
      <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Track your progress towards major milestones.</p>
        </div>
      </div>
            ...
            <div key={g.id} className="relative">
              <div className="flex justify-between items-end mb-2">
                <div>
                  ...
                  <div className="text-xs text-slate-500 font-medium flex gap-2 items-center mt-1">
                    <TrendingUp size={14} className={isComplete ? "text-emerald-500" : "text-indigo-400"} />
                    ₹{g.saved.toLocaleString('en-IN')} / ₹{g.target.toLocaleString('en-IN')}
                  </div>
                </div>
                <div className="text-right">
                  <span className={`text-sm font-bold block ${isComplete ? 'text-emerald-600' : 'text-slate-700'}`}>
                    {percent.toFixed(1)}%
                  </span>
