...
export default function Simulation() {
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">AI Advisor & Simulator</h1>
        <p className="text-slate-500 mt-1">Leverage AI to optimize your wealth and debt strategies.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4">
          <button onClick={() => handleAskAI('general')} className="w-full text-left p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-colors shadow-sm flex items-start gap-3">
            <div className="p-2 bg-indigo-100 text-indigo-600 rounded-lg"><Sparkles size={20} /></div>
            <div>
              <h3 className="font-semibold text-slate-900">General Health Check</h3>
              <p className="text-xs text-slate-500 mt-1">Get 3 immediate actionable insights.</p>
            </div>
          </button>
          
          <button onClick={() => handleAskAI('debt')} className="w-full text-left p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-colors shadow-sm flex items-start gap-3">
            <div className="p-2 bg-rose-100 text-rose-600 rounded-lg"><Bot size={20} /></div>
            <div>
              <h3 className="font-semibold text-slate-900">Debt Destroyer Strategy</h3>
              <p className="text-xs text-slate-500 mt-1">Mathematical analysis on what to pre-close.</p>
            </div>
          </button>

          <button onClick={() => handleAskAI('fire')} className="w-full text-left p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-colors shadow-sm flex items-start gap-3">
            <div className="p-2 bg-emerald-100 text-emerald-600 rounded-lg"><Sparkles size={20} /></div>
            <div>
              <h3 className="font-semibold text-slate-900">FIRE Readiness</h3>
              <p className="text-xs text-slate-500 mt-1">Calculate your retirement corpus & date.</p>
            </div>
          </button>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm min-h-[400px] flex flex-col">
            <h2 className="text-lg font-semibold mb-4 border-b pb-2">AI Output</h2>
            <div className="flex-1 overflow-auto prose prose-slate max-w-none text-sm">
              {loading && (
